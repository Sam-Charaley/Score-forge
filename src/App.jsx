...
function App() {
  ...
  useEffect(() => {
    ...
    Auth.onAuthChange(async (sessionUser) => {
      if (sessionUser) { await loadUserData(sessionUser); }
      else { setUser(null); setProfile(null); setChecked({}); setSelectedIds(null); setPlanSettings({}); }
      setLoading(false);
    });
  }, []);
  ...
  const loadUserData = async (sessionUser) => {
    setUser(sessionUser);
    const [prof, subs, prog, plans, str] = await Promise.all([
      Auth.getProfile(sessionUser.id),
  ...
  const handlePickerSave = async (ids, newPlanSettings) => {
    ...
    await Promise.all(Object.entries(newPlanSettings).map(([sid, setting]) => Auth.savePlanSetting(user.id, sid, setting)));
